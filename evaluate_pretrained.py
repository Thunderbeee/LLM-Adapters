...
def parse_args():
    ...
    parser.add_argument('--dataset', choices=['AddSub', 'MultiArith', 'SingleEq', 'gsm8k', 'AQuA', 'SVAMP'],
                        required=True)
    parser.add_argument('--model', choices=['LLaMA-7B', 'LLaMA2-7B', 'LLaMA2-13B', 'BLOOM-7B', 'GPT-j-6B'], required=True)
...
def load_model(args) -> tuple:
    ...
    if args.model in ['LLaMA-7B', 'LLaMA2-7B','LLaMA2-13B']:
        token = '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'
...
def extract_answer_number(args, sentence: str) -> float:
    ...
    assert dataset in ["multiarith", "addsub", "singleeq", "gsm8k", "svamp"]
